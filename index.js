...
*/
  ...
  .then((cellValues) => {
    ...
    zeile.shift();
    for (let i = 1; i <= 21; i++) {
      const dynamischeAdresse = "Etikette" + i;
      const naechstesElement = zeile.shift();
      patchObject[dynamischeAdresse] = {
        type: PatchType.DOCUMENT,
        children: [
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: naechstesElement.schemaNr,
                bold: true, // Alle Durchläufe machen den ersten Paragraphen fett
              }),
            ],
          }),
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun(naechstesElement.function)],
          }),
          new Paragraph(""), // Leerer Paragraph für einen Zeilenabstand
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun(naechstesElement.typ)],
          }),
        ],
      };
    }
